...
class TechPicker(BaseAgent):
    ...
    def process(self, state: dict) -> dict:
        ...
        try:
            ...
            # 拼接 categories 数组为正则字符串（循环处理每个 item）
            if isinstance(state["categories"], list):
                categories_regex = "|".join(f"{item}|{item} | {item}" for item in state["categories"])
            else:
                categories_regex = f"{state['categories']}|{state['categories']} | {state['categories']}"

            query = {
              "$or": [
                  {
                    "categories": { 
                      "$regex": categories_regex,
                      "$options": "i" 
                    }         
                  }, {
                    "title": {
            ...
            results = self.db_client.get_raw_data_collection().find(query)
            
            # 方法1：使用 list() 函数
            articles = list(results)
            
            # 方法2：使用列表推导式
            # articles = [doc for doc in results]
            
            # 方法3：使用 to_list() 方法
            # articles = results.to_list()
            
            print(f"Successfully get category: {articles}")

            return { **state, "pick_article": articles }
