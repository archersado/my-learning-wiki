"""Ingest MAS lesson markdown files into wiki/mas/ using the project's wiki infrastructure."""
...
SYSTEM_PROMPT = """You are a knowledge extraction expert. Given a lesson document from a multi-agent systems curriculum, extract the following as JSON:
...
def extract_info(llm, content: str, title: str) -> dict:
    """Call LLM to extract entities, concepts, and summary from lesson content."""
...
def find_lesson_file(lessons_dir: Path, lesson_dir: str) -> Path | None:
    """Find the lesson markdown file within a lesson directory."""
    candidates = [
    ...
    # Also check any .md file directly in the lesson dir
    for candidate in candidates:
        if candidate.exists():
            return candidate
    # Fallback: grab any .md file in the lesson directory
...
def ingest_lessons():
    """Ingest all MAS lessons into wiki/mas/."""
    llm = build_gpt4o()

    # Resolve paths relative to the project root (content-operation/)
    project_root = Path(__file__).resolve().parent.parent.parent  # src/scripts -> src -> content-operation
    lessons_dir = project_root / "learn/ai-engineering-from-scratch/phases/16-multi-agent-and-swarms"

    # Set up wiki components
    mas_wiki_root = str(project_root / "wiki" / "mas")
    ...
    print(f"MAS wiki root: {mas_wiki_root}")
    print(f"Lessons directory: {lessons_dir}")

    # Dynamically discover lesson directories (sorted, skip non-lesson dirs)
    lesson_dirs = sorted([
    ...
    for lesson_dir in lesson_dirs:
        ...
        for entity in extraction.get("entities", []):
            ...
            if matches:
                ...
                cre.create_entity_page(entity, "Entity mentioned in MAS curriculum.", [lesson_title])
        ...
        for concept in extraction.get("concepts", []):
            ...
            if matches:
                ...
                cre.create_concept_page(concept, "Concept from MAS curriculum.", [lesson_title])
    ...
    lm.append("ingest (batch)", f"{count} MAS lessons ingested", "MAS wiki creation completed")
    print(f"\nDone: {count}/{len(lesson_dirs)} lessons ingested into {mas_wiki_root}")
